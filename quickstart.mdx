...
MemSync uses JWT token-based authentication for all API requests. You'll need to include your JWT token in the Authorization header:
...
<Note>
  **Coming Soon**: App-specific API keys for enhanced security and easier integration.
</Note>
...
# Usage
chatbot = PersonalizedChatbot("YOUR_JWT_TOKEN")
...
<AccordionGroup>
  <Accordion title="401 Unauthorized Error">
    Make sure your JWT token is valid and included in the Authorization header as `Bearer YOUR_TOKEN`.
  </Accordion>
